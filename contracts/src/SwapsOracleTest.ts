import { SwapsOracle } from './SwapsOracle';
import {
  isReady,
  shutdown,
  Field,
...
describe('SwapsOracle', () => {
  ...
  beforeAll(async () => {
    await isReady;
    if (proofsEnabled) SwapsOracle.compile();
  });
  ...
  it('generates and deploys the oracle smart contract', async () => {
    const zkAppInstance = new SwapsOracle(zkAppAddress);
  ...
  describe('actual API requests', () => {
    it('emits an `id` event containing the transaction id if the wallet and transaction match the oracle', async () => {
      const zkAppInstance = new SwapsOracle(zkAppAddress);
      await localDeploy(zkAppInstance, zkAppPrivateKey, deployerAccount);

      const response = await fetch(
        'http://localhost:9000/transaction/0x07aac1d5997e4cb1c6bd88b15ab4500ef7dfe0b70919fdb69da62d9398e0b7bf'
      );
      ...
      const walletId = Field(data.data.walletId);
      ...
      try {
        ...
        console.error('Caught error during Mina.transaction', err);
      }

      // Subsequent code using txn
      if (txn) {
        await txn.prove();
        await txn.send();
